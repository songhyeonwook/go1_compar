# go1_compar — 3-paradigm baseline comparison (self-contained)

논문 §4.7 / §2.3·§2.5의 **비교군(baseline)** 실험. 제안 방법(antalgic)이 부상동물
biomechanics를 재현하는 것이 **통각 보상 때문**임을, **동일한 환경·구조에서 환부 다리
보상항만 바꾼** 3개 패러다임 비교로 입증한다. **서버에서 clone → 실행**할 수 있도록
환경·파이프라인·baseline 코드를 모두 포함한 self-contained repo.
...
scripts/rsl_rl/          학습·평가 파이프라인 (train.py, train_phase2_stable.sh 체인,
                         ...
                         aggregate_nseed.py)
baselines/               비교군 실행 스크립트
  launch_compar.sh       한 패러다임 teacher 1개 학습 (foreground)
  run_baselines.sh       3개 패러다임 학습 → 자동 평가·비교 (detached로 실행)
  eval_compar.sh         3 패러다임 저속평가 → biomech 덤프 → 비교표
